...
def intcomp(listdata):
    for x in range(len(listdata)):
        ...
        op = code[len(code)-1] + (code[len(code)-2] * 10)
            if op == 1:
                listdata[getvalue(listdata,code[len(code)-5],x+3)] = getvalue(listdata,code[len(code)-4],x+2) + getvalue(listdata,code[len(code)-3],x+1)
                x += 4
#               print(listdata)
            elif op == 2:
                listdata[getvalue(listdata,code[len(code)-5],x+3)] = getvalue(listdata,code[len(code)-4],x+2) * getvalue(listdata,code[len(code)-3],x+1)
